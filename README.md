...
```
index.html          # Entry point — loads p5.js and blob.js
blob.js             # Sketch: animated organic blob
```
...
**Object hierarchy**: `Cluster` → `Blob[]` → `Point[]`. Each `Blob` has 5 vertices that oscillate between min/max radii using acceleration physics. `Cluster` holds multiple `Blob` layers drawn with low opacity to create depth.
